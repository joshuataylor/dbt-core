...
fn parse_knobs(mut input: ItemFn, is_test: bool, config: FinalConfig) -> TokenStream {
    ...
    let dbt_runtime_crate = Ident::new(
        "dbt_runtime",
        Span::call_site().located_at(last_stmt_start_span),
    );
    ...
    let (pre_return, callbacks) = match config.flavor {
        RuntimeFlavor::Threaded => (
            quote_spanned! {last_stmt_start_span=>
                let __dbt_rt_enter = __dbt_rt.handle().enter();
                let __dbt_rt_handle = __dbt_rt.handle().clone();
            },
            quote_spanned! {last_stmt_start_span=>
                .on_thread_start(move || {
                    let guard = unsafe { __dbt_rt_handle.enter_owned() };
                    __DBT_RT_GUARD.set(Some(guard));
                })
                .on_thread_stop(|| {
                    __DBT_RT_GUARD.set(None);
                })
            },
        ),
        RuntimeFlavor::CurrentThread | RuntimeFlavor::Local => (
    ...
    let last_block = quote_spanned! {last_stmt_end_span=>
        ...
        #[allow(clippy::expect_used, clippy::diverging_sub_expression, clippy::needless_return, clippy::unwrap_in_result)]
        {
            ...
            thread_local! {
                static __DBT_RT_GUARD: Cell<Option<#dbt_runtime_crate::SetCurrentGuard>> =
                    const { Cell::new(None) };
            }

            let __dbt_rt = #dbt_runtime_crate::builder::Builder::new()
