...
use crate::task::{Schedule, TaskHarnessScheduleHooks};
...
impl<T: Future, S: Schedule> Cell<T, S> {
    ...
    pub(super) fn new(
        ...
        let result = Box::new(Cell {
            ...
            header: new_header(state, vtable, tracing_id),
            core: Core {
                scheduler,
                stage: CoreStage {
...
mod tests {
    ...
    use crate::blocking_task::BlockingTask;
    use crate::schedule::BlockingSchedule;
    use crate::util::trace::{SpawnMeta, blocking_task};

    type TestCell<F> = Box<Cell<Instrumented<BlockingTask<F>>, BlockingSchedule>>;

    /// A `BlockingSchedule` needs a runtime handle to read its hooks off, and
    /// the future has to be the instrumented one, since `Cell::new` reads
    /// `Header::tracing_id` off it. Both are as at a real spawn site; building a
    /// runtime spawns no threads.
    fn cell_of<F, R>(func: F, id: Id) -> TestCell<F>
    where
        F: FnOnce() -> R + Send + 'static,
        R: Send + 'static,
    {
        let rt = crate::builder::Builder::new().build();
        let task = blocking_task::<F, BlockingTask<F>>(
            BlockingTask::new(func),
            SpawnMeta::new_unnamed(size_of::<F>()),
            id.as_u64(),
        );
        Cell::new(
            task,
    ...
    /// `RawTask::new` casts `*mut Cell` straight to `*mut Header`, so the
    /// header has to be the first field. Nothing else enforces this.
    #[test]
    fn header_is_the_first_field_of_cell() {
        let cell = cell_of(|| 7u32, Id::next());

        assert_eq!(
            &cell.header as *const Header as usize,
            &*cell as *const Cell<_, _> as usize,
        );
    }

    /// The `const` offsets in `raw.rs` are computed from the `#[repr(C)]`
    /// layout by hand. This checks they agree with what the compiler actually
    /// laid out. `Cell::new` asserts the same thing under `debug_assertions`;
    /// this makes it run, and states it independently.
    #[test]
    fn vtable_offsets_match_the_real_layout() {
        let cell = cell_of(|| 7u32, Id::next());
        let header = NonNull::from(&cell.header);

        unsafe {
            assert_eq!(
                Header::get_trailer(header).as_ptr() as usize,
                &cell.trailer as *const Trailer as usize,
                "trailer_offset"
            );
            assert_eq!(
                Header::get_scheduler::<BlockingSchedule>(header).as_ptr() as usize,
                &cell.core.scheduler as *const BlockingSchedule as usize,
                "scheduler_offset"
            );
            assert_eq!(
                Header::get_id_ptr(header).as_ptr() as usize,
                &cell.core.task_id as *const Id as usize,
                "id_offset"
            );
            assert_eq!(
                Header::get_spawn_location_ptr(header).as_ptr() as usize,
                &cell.core.spawned_at as *const &'static Location<'static> as usize,
    ...
    #[test]
    fn header_reads_back_the_id_and_spawn_location() {
        let id = Id::next();
        let cell = cell_of(|| 7u32, id);
        let header = NonNull::from(&cell.header);

        unsafe {
            assert_eq!(Header::get_id(header), id);
            assert_eq!(
                Header::get_spawn_location(header).file(),
                file!(),
                "the location captured in `cell_of`"
            );
        }
    }

    /// Polling runs the closure and leaves the stage consumed.
    #[test]
    fn poll_runs_the_blocking_closure() {
        let cell = cell_of(|| 7u32, Id::next());

        let waker = Waker::noop();
        let res = cell.core.poll(Context::from_waker(waker));

        assert!(matches!(res, Poll::Ready(7)));
    }

    /// A blocking task's closure runs *inside* `Core::poll`, so `TaskIdGuard`
    /// is live for the whole of it. This is why `task::id()` works inside
    /// blocking work.
    #[test]
    fn the_task_id_is_current_while_the_closure_runs() {
        let id = Id::next();
        ...
        let cell = {
            ...
            cell_of(
                move || *seen.lock().unwrap() = crate::task::id::try_id(),
                id,
            )
        };

        assert_eq!(crate::task::id::try_id(), None, "not inside a task yet");

        let waker = Waker::noop();
        let _ = cell.core.poll(Context::from_waker(waker));

        assert_eq!(*seen.lock().unwrap(), Some(id));
        assert_eq!(
            crate::task::id::try_id(),
            None,
            "the guard restored the previous id on the way out"
        );
    }

    #[test]
    fn output_round_trips_through_the_stage() {
        let cell = cell_of(|| 7u32, Id::next());

        cell.core.store_output(Ok(7));
        assert_eq!(cell.core.take_output().unwrap(), 7);
    }

    #[test]
    #[should_panic(expected = "JoinHandle polled after completion")]
    fn taking_the_output_twice_panics() {
        let cell = cell_of(|| 7u32, Id::next());

        cell.core.store_output(Ok(7));
        let _ = cell.core.take_output();
        let _ = cell.core.take_output();
    }
}
