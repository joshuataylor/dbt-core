//! Mirrors tokio's `runtime/task/raw.rs`: an untyped pointer to a task
//! allocation, plus the vtable that erases `T` and `S`.
//!
//! The layout arithmetic is ported in full — the `const` offsets here and the
//! `#[repr(C)]` layout in `core.rs` have to agree, and `Cell::new`'s
//! `debug_assertions` block checks that they do.
//!
//! Dropped vs upstream: `drop_abort_handle` (no `AbortHandle` yet) and
//! `get_queue_next`/`set_queue_next` (injection-queue bookkeeping, gone with
//! the no-`list.rs` decision).

// It doesn't make sense to enforce `unsafe_op_in_unsafe_fn` for this module
// because
//
// * This module is doing the low-level task management that requires tons of
//   unsafe operations.
...
use crate::task::core::{Core, Header, Trailer};
use crate::task::harness::Harness;
use crate::task::{Result, Schedule, SpawnLocation};
...
pub(super) struct Vtable {
    ...
    ///
    /// Unreachable for a blocking task, which is `Ready` on its first poll.
    /// Kept so that a stray wake trips `BlockingSchedule::schedule`'s
    /// `unreachable!()` rather than being silently dropped.
...
impl RawTask {
    ...
    pub(super) fn header(&self) -> &Header {
        // Safety: the pointer is valid for the lifetime of the task.
    ...
    pub(super) fn state(&self) -> &crate::task::state::State {
        &self.header().state
    }
    ...
    pub(super) unsafe fn try_read_output<O>(self, dst: *mut Poll<Result<O>>, waker: &Waker) {
        let vtable = self.header().vtable;
...
// ===== vtable entry points =====
//
// Each of these erases `T` and `S` and hands off to `Harness`.
...
/// Unreachable for a blocking task: `BlockingTask` is `Ready` on its first
/// poll, so it is never woken and never rescheduled. Wired up anyway so a
/// stray wake reaches `BlockingSchedule::schedule`'s `unreachable!()`.
...
unsafe fn try_read_output<T: Future, S: Schedule>(
    ...
    let out = &mut *(dst as *mut Poll<Result<T::Output>>);
