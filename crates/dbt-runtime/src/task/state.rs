//! Mirrors tokio's `runtime/task/state.rs`: the atomic bitfield tracking a
//! task's lifecycle and ref-count.
...
///
///  * A reference held by the `Task` handle. Upstream this goes into an
///    `OwnedTasks` list; here `unowned()` folds it into the `UnownedTask`,
///    which is why that type holds two ref-counts.
///  * A reference for the `Notified`, likewise folded into the `UnownedTask`.
...
mod tests {
    ...
    /// The invariant the whole file rests on: the lifecycle/flag bits and the
    /// ref-count share one word without overlapping.
    #[test]
    fn state_and_ref_count_bits_are_disjoint() {
        assert_eq!(STATE_MASK & REF_COUNT_MASK, 0);
        assert_eq!(STATE_MASK | REF_COUNT_MASK, usize::MAX);
        assert_eq!(REF_ONE, 1 << REF_COUNT_SHIFT);
        // One ref count must live entirely in the ref-count half.
        assert_eq!(REF_ONE & STATE_MASK, 0);
    }

    /// `unowned()` hands out three references: two folded into the
    /// `UnownedTask`, one for the `JoinHandle`.
    #[test]
    fn initial_state() {
        let state = State::new();
    ...
    #[test]
    fn ref_dec_reports_last_reference() {
        let state = State::new();

        assert!(!state.ref_dec(), "3 -> 2 is not the last");
        assert!(!state.ref_dec(), "2 -> 1 is not the last");
        assert!(state.ref_dec(), "1 -> 0 is the last");
    }

    /// `UnownedTask` holds two ref-counts, so it drops both at once.
    #[test]
    fn ref_dec_twice_reports_last_reference() {
        let state = State::new();
        assert!(!state.ref_dec_twice(), "3 -> 1 is not the last");
        assert_eq!(state.load().ref_count(), 1);

        let state = State::new();
        state.ref_dec();
        assert!(state.ref_dec_twice(), "2 -> 0 is the last");
    }

    #[test]
    fn ref_inc_then_dec_round_trips() {
        let state = State::new();
        state.ref_inc();
        assert_eq!(state.load().ref_count(), 4);
        assert!(!state.ref_dec());
        assert_eq!(state.load().ref_count(), 3);
    }

    /// The happy path a blocking task actually takes: notified -> running ->
    /// complete. It is never rescheduled, so it goes straight to complete
    /// rather than back to idle.
    #[test]
    fn blocking_task_lifecycle() {
        let state = State::new();
        ...
        assert!(
            ...
            "the notified bit is consumed by the transition, so a wake during \
             the poll is detectable"
        );
    ...
    /// `Task::shutdown` on a task that never ran. Note it does not merely
    /// flag the task: it *takes the running lock*, so the caller shutting the
    /// pool down is the one that completes it.
    #[test]
    fn transition_to_shutdown_on_an_idle_task_takes_the_running_lock() {
        let state = State::new();

        assert!(
            state.transition_to_shutdown(),
            "true means the caller now owns completing this task"
        );
        ...
        assert!(
            snapshot.is_running(),
            "shutdown locked RUNNING on the caller's behalf"
        );
    }

    /// Follows on from the above: a worker that later picks the task up must
    /// not be able to run it, because shutdown already holds the lock.
    #[test]
    fn transition_to_running_after_shutdown_fails() {
        let state = State::new();
        assert!(state.transition_to_shutdown());

        assert!(
            matches!(state.transition_to_running(), TransitionToRunning::Failed),
            "the task is no longer idle, so the ref-count is consumed and the \
             attempt fails"
        );
    ...
    #[test]
    fn transition_to_shutdown_on_a_running_task_defers_to_the_runner() {
        let state = State::new();
        let _ = state.transition_to_running();

        assert!(
            !state.transition_to_shutdown(),
            "the thread running the task is responsible for completing it"
        );
        assert!(state.load().is_cancelled());
    }

    #[test]
    fn transition_to_terminal_reports_when_to_dealloc() {
        let state = State::new();
        assert!(!state.transition_to_terminal(2), "3 - 2 = 1 left");
        assert!(state.transition_to_terminal(1), "1 - 1 = 0, dealloc");
    }

    /// The uncontended `JoinHandle` drop: nothing has touched the task yet, so
    /// one CAS drops the join interest and its ref-count.
    #[test]
    fn drop_join_handle_fast_succeeds_on_a_pristine_task() {
        let state = State::new();

        assert!(state.drop_join_handle_fast().is_ok());
    ...
    #[test]
    fn drop_join_handle_fast_fails_once_the_task_has_moved() {
        let state = State::new();
        let _ = state.transition_to_running();

        assert!(
            state.drop_join_handle_fast().is_err(),
            "the fast path must only fire on the initial state"
        );
    }

    #[test]
    fn join_waker_can_be_set_and_unset() {
        let state = State::new();
        assert!(!state.load().is_join_waker_set());

        assert!(state.set_join_waker().is_ok());
        assert!(state.load().is_join_waker_set());

        assert!(state.unset_waker().is_ok());
        assert!(!state.load().is_join_waker_set());
    }

    #[test]
    fn setting_a_join_waker_on_a_complete_task_fails() {
        let state = State::new();
        let _ = state.transition_to_running();
        let _ = state.transition_to_complete();

        assert!(
            state.set_join_waker().is_err(),
            "the output is already there; the caller must read it instead"
        );
    }
}
